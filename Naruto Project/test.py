...
SEGMENT_EVERY_N_FRAME = 3

# Clone behavior
# dx, dy are in pixels here, easier to tune visually
CLONE_SPECS = [
    {"dx": -170, "dy":  70, "scale": 0.96, "alpha": 1.00, "delay": 0.04},
    {"dx":  170, "dy":  70, "scale": 0.96, "alpha": 1.00, "delay": 0.08},
    {"dx": -300, "dy":  95, "scale": 0.84, "alpha": 0.82, "delay": 0.14},
    {"dx":  300, "dy":  95, "scale": 0.84, "alpha": 0.82, "delay": 0.18},
    {"dx":  430, "dy":  130, "scale": 0.68, "alpha": 0.58, "delay": 0.24},
    {"dx": -430, "dy":  130, "scale": 0.68, "alpha": 0.58, "delay": 0.30},
...
def main():
    ...
    flash_until = 0.0
    cached_alpha_mask = None
    frame_index = 0

    last_hand_timestamp_ms = -1
    last_segment_timestamp_ms = -1
    ...
    with HandLandmarker.create_from_options(hand_options) as landmarker, \
        ...
        while True:
            ...
            hand_timestamp_ms = max(now_ms, last_hand_timestamp_ms + 1)
            last_hand_timestamp_ms = hand_timestamp_ms

            hand_result = landmarker.detect_for_video(hand_image, hand_timestamp_ms)
            ...
            now = time.time()
            pose_ok = both_hands_ok and avg_shadow_conf > CONF_THRESHOLD
            ...
            if now < activated_until:
                if cached_alpha_mask is None or frame_index %  SEGMENT_EVERY_N_FRAME == 0:
                     segment_timestamp_ms = max(now_ms, last_segment_timestamp_ms + 1)
                     last_segment_timestamp_ms = segment_timestamp_ms
                     cached_alpha_mask = get_person_alpha_mask(segmenter, raw_frame, segment_timestamp_ms)

                alpha_mask = cached_alpha_mask

                person_bgr = (raw_frame.astype(np.float32) * alpha_mask[..., None]).astype(np.uint8)

                activation_progress = (now - activation_started_at) / ACTIVATION_SECONDS
        ...
        #        )
            ...
            #frame = smoke_effect(frame, smoke_img, 50, 50, alpha_mult=1.0)
